...
function Thumbnail({ video, onClick }: { video: SanityVideo; onClick: () => void }) {
  return (
    <div
...
export default function VideosClient({ videos }: { videos: SanityVideo[] }) {
  const { t } = useI18n();
      ...
      <section className="py-20 bg-zinc-950">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {videos.map((video, i) => (
              <Thumbnail key={video._id} video={video} onClick={() => setSelected(i)} />
            ))}
          </div>
        ...
        >
          ...
          >
            <div className="w-full aspect-video">
              {current.videoType === "youtube" && current.youtubeId ? (
                <iframe
