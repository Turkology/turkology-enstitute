...
export default function PhotosClient({ photos }: { photos: SanityPhoto[] }) {
      ...
      <section className="py-20 bg-zinc-950">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {photos.map((photo, i) => {
              const caption = locale === "tr" ? photo.caption_tr : photo.caption_en;
              const imgUrl = urlFor(photo.image).width(800).url();
              return (
                <div
                  key={photo._id}
        ...
        >
          ...
          >
            <div className="relative w-full h-full max-h-[75vh]">
              <Image
                src={urlFor(photos[selected].image).width(1600).url()}
                alt={locale === "tr" ? photos[selected].caption_tr : photos[selected].caption_en}
                fill
          ...
          >
            <p className="text-white font-medium text-center">
              {locale === "tr" ? photos[selected].caption_tr : photos[selected].caption_en}
            </p>
